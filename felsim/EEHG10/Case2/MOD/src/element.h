...
{
	private:
		double undulator_period, undulator_field, undulator_phi0;
		unsigned int undulator_num, undulator_nstep;
		bool undulator_idmpx;
	public:
		void set_field (double x); //x: [tesla], undulator peak field
		void set_phi0  (double x); //x: [rad], initial phase value
