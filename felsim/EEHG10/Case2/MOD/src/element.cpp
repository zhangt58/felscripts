...
void undulator::set_phi0(double x)
{
	undulator_phi0 = x*PI; // initial phase, [pi rad]
}
...
void undulator::set_idmpx(bool flag)
{
	undulator_idmpx = flag; // dump x trajectory or not (big data block)
}
...
double undulator::get_phi0()
{
	return undulator_phi0; // initial phase, [pi rad]
}
...
bool undulator::get_idmpx()
{
	return undulator_idmpx; // dump x traj or not
}
...
void undulator::printall()
{
	...
	std::cout << std::setw(16) << "initial phase: "  << std::setw(10) << undulator_phi0   << " [pi]\n";
	...
	std::cout << std::setw(16) << "dump x traj?: " 	 << std::setw(10) << undulator_idmpx  << std::endl;
	std::cout << "------------------------------\n\n";
}
