...
void readdata(std::ifstream &infile, elementType &param_eletype, double* &s0, double* &gam0, double* &x0, double* &y0, double* &vx0, double* &vy0)
{
	...
	{
		infile >> ptrs0[i]; ptrs0[i]*=C0; 	// t [sec] -> s [m]
		infile >> ptrgam0[i]; 				// gamma
		infile >> ptrx0[i]; 				// x [m]
		infile >> ptry0[i]; 				// y [m]
		infile >> ptrvx0[i]; ptrvx0[i]*=C0; // beta_x -> vx [m/s]
		infile >> ptrvy0[i]; ptrvy0[i]*=C0; // beta_y -> vy [m/s]
	}
	infile.close();
	s0   = ptrs0;
	gam0 = ptrgam0;
...
//	double zc = 0;
	...
	else if(flag == 1)//undulator
	{
		...
		double              phi0= param_undulator.get_phi0  ();
		bool 			   idmpx= param_undulator.get_idmpx ();
		double undulator_period = param_undulator.get_period();
//		std::cout << phi0 << std::endl;
		double By, ku = 2.0*PI/undulator_period;
		if(idmpx == 1) // dump x trajectories of all particles
		{
			double *traj_x = new double [nstep*Nu];
			std::ofstream trajxfile("trajx.dat", std::ios::binary);
			...
			for(unsigned int i = 0; i< npart; i++)
			{
				...
				{
					By   = B0*sin(ku*z+PI/2-phi0); // initial phase phi0
					Ex   = param_seed.get_ElectricalFieldIntensityz(z0,zc,x,y);
					ax   = E0/gam/M0*(-vz*By);
					az   = E0/gam/M0*(vx*By);
					x   += (vx+0.5*ax*dt)*dt;
					...
					vx  += (ax*dt);
					vz   = sqrt(1.0-1.0/gam/gam-(vx*vx+vy*vy)/C0/C0)*C0;
					traj_x[j] = x; // store trajectory of i-th particle
				}
				
				// write traj_x into file "trajx.dat"
				for(unsigned int ik=0;ik<nstep*Nu;ik++)
					trajxfile.write((char*)(&traj_x[ik]),sizeof(double));

				// write i-th particle to outfile
				outfile.precision(18);
				outfile << z0/C0 << "\t" << gam << "\t" << x << "\t" << y << "\t" << vx/C0 << "\t" << vy/C0 << "\n" ;
				sum_gsq += (gam*gam);
				sum_g   += gam;
