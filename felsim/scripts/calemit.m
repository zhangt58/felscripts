...
deg  = zeros(nslice,1);
for i = 1:nslice
	fid = fopen(strcat('./NoCSR/mslices/slice',num2str(i)));
	a = fread(fid,'double');
...
figure
plot(1:nslice,deg)
dlmwrite('emitnxy.dat',[enx,eny],'delimiter',' ','precision','%.6e')
dlmwrite('bf13.dat',[bf1,bf3],'delimiter',' ','precision','%.6e')
dlmwrite('gam.dat',[gam,deg],'delimiter',' ','precision','%.6e')
